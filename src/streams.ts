...
export interface EmitOptions<V,R> {
  // cacheName?: string
  noThrow?: boolean
  map?: (v: V) => R
}

// export type EmitType<V,R> =
...
export class SignalWrap<S extends {}> {
  ...
  emit = <K extends keyof S, T> (k: K, opt?: EmitOptions<K, T>): AsCallbacks<S>[K] =>
    this._cb[k] || _throw(ErrorCode.SIGNAL_TO_VOID)
