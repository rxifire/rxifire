...
import { mockActions, Actions, ActionsIO } from '../actions'
...
type Spec = F$.ComponentSpec<State, Signals, ActionsIO,
...
const log: F$.Logic<Spec> = ({ beh, sig, act }) => {
  ...
  return Observable.of({ name: 'ab', pos: { x: 0, y: 0 } })
    .merge($.timer(0, 200).map(x => ({ name: `${x}-`, pos: { x, y: -x } })))
    .merge($.timer(2000).mergeMap(() => act.fire('randomNumbers')(20)).ignoreElements())
    .delay(1)
}
